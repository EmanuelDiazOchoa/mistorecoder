...
const CATEGORY_COLORS: Record<string, { primary: string; bg: string; border: string }> = {
...
function CategoryCard({ item, index, onPress }: { item: any; index: number; onPress: (item: any) => void }) {
...
export default function CategoriesScreen({ navigation }: { navigation: any }) {
  ...
  const handlePress = (item: { key: string }) =>
    navigation.navigate('CategoryProducts', { category: item.key });
...
const styles = StyleSheet.create({
  ...
  emojiWrap: { width: 64, height: 64, borderRadius: 20, alignItems: 'center', justifyContent: 'center' },
  emoji: { fontSize: 34 },
