...
function CheckIcon({ color, size = 56 }: { color: string; size?: number }) {
  return (
...
function IconBag({ color, size = 18 }: { color: string; size?: number }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
function IconHome({ color, size = 18 }: { color: string; size?: number }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      ...
      <Animated.View style={[
        ...
        <View style={[styles.ring, { borderColor: accentColor, backgroundColor: `${accentColor}15` }]}>
          <Animated.View style={{ transform: [{ scale: checkScale }] }}>
            <CheckIcon color={accentColor} size={56} />
          </Animated.View>
        </View>
      </Animated.View>
          ...
          return (
            <View key={step.label} style={styles.stepItem}>
              <View style={styles.stepDotCol}>
                ...
                {!isLast && (
                  <View style={[styles.stepConnector, { backgroundColor: isDone ? accentColor : 'rgba(255,255,255,0.08)' }]} />
                )}
              </View>
              <Text style={[
                styles.stepLabel,
...
const styles = StyleSheet.create({
  ...
  stepDotCol:    { alignItems: 'center', marginBottom: 6 },
  ...
  stepLabel:     { fontSize: 10, fontWeight: '600', textAlign: 'center', letterSpacing: 0.2 },
