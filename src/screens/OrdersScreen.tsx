...
function OrderCard({ item, index, total, accentColor }: OrderCardProps) {
  ...
  return (
    <Animated.View style={[styles.card, {
      ...
      {item.items.map((prod: CartItem, i: number) => (
        <View key={i} style={styles.productRow}>
          <Text style={styles.productName}>
            • {prod.name.charAt(0).toUpperCase() + prod.name.slice(1)}
            {prod.quantity > 1 ? ` x${prod.quantity}` : ''}
          </Text>
          <Text style={styles.productPrice}>${(prod.price * (prod.quantity || 1)).toFixed(2)}</Text>
        </View>
      ))}
      <View style={styles.divider} />
      ...
      <View style={styles.totalRow}>
        ...
        <Text style={[styles.total, { color: accentColor }]}>${item.total?.toFixed(2)}</Text>
      </View>
    </Animated.View>
  );
}
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <Animated.View style={[styles.header, {
        ...
        {orders.length > 0 && (
          <View style={[styles.countBadge, { backgroundColor: `${theme.primary}20`, borderColor: `${theme.primary}40` }]}>
            <Text style={[styles.countText, { color: theme.primary }]}>{orders.length}</Text>
          </View>
        )}
      </Animated.View>
      ...
      {orders.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <OrderCard item={item} index={index} total={orders.length} accentColor={theme.primary} />
          )}
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  bgGlow: { position: 'absolute', width: 250, height: 250, borderRadius: 125, opacity: 0.06, top: -50, right: -60 },
  bgGlow2: { position: 'absolute', width: 200, height: 200, borderRadius: 100, backgroundColor: '#7C3AED', opacity: 0.05, bottom: 100, left: -40 },
  header: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingTop: 56, paddingBottom: 20, paddingHorizontal: 24 },
  ...
  card: { backgroundColor: 'rgba(255,255,255,0.04)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', borderRadius: 24, padding: 20, marginBottom: 16 },
  ...
  statusBadge: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: 'rgba(39,174,96,0.15)', borderWidth: 1, borderColor: 'rgba(39,174,96,0.3)', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 5 },
  ...
  productName: { fontSize: 14, color: 'rgba(255,255,255,0.6)', flex: 1, marginRight: 8 },
